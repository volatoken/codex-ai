...
if (!(Get-Command git -ErrorAction SilentlyContinue)) {
    ...
    $gitVer = git --version
    Write-Host "  Git da co san: $gitVer" -ForegroundColor Green
}
...
if (!(Get-Command cargo -ErrorAction SilentlyContinue)) {
    ...
    Write-Host "  Rust da cai xong: $rustVer" -ForegroundColor Green
} else {
    ...
    if ($currentToolchain -like '*gnu*') {
        ...
        $rustVer = rustc --version
        Write-Host "  Da chuyen sang MSVC: $rustVer" -ForegroundColor Green
    } else {
        $rustVer = rustc --version
        Write-Host "  Rust da co san (MSVC): $rustVer" -ForegroundColor Green
    }
}
...
if (!(Get-Command python -ErrorAction SilentlyContinue)) {
    ...
    Write-Host "  Python da cai xong: $pyVer" -ForegroundColor Green
} else {
    $pyVer = python --version
    Write-Host "  Python da co san: $pyVer" -ForegroundColor Green
}
...
if (!(Get-Command docker -ErrorAction SilentlyContinue)) {
    ...
    try {
        ...
        Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/microsoft/Windows-Containers/Main/helpful_tools/Install-DockerCE/install-docker-ce.ps1' -OutFile $dockerScript
        & $dockerScript -NoRestart
        Remove-Item $dockerScript -Force -ErrorAction SilentlyContinue
        Refresh-Path
    ...
    if (!$dockerInstallOk) {
        ...
        $dockerDest = $env:ProgramFiles
        $dockerDir = Join-Path $dockerDest 'Docker'
        Invoke-WebRequest -Uri $dockerUrl -OutFile $dockerZip
        Expand-Archive -Path $dockerZip -DestinationPath $dockerDest -Force
        Remove-Item $dockerZip -Force -ErrorAction SilentlyContinue

        # Them vao PATH
        $machinePath = [System.Environment]::GetEnvironmentVariable('Path','Machine')
        if ($machinePath -notlike '*Docker*') {
            $newPath = $machinePath + ';' + $dockerDir
        ...
        & $dockerdExe --register-service 2>&1 | Out-Null
        Start-Service docker
    }

    $dockerVer = docker --version
    Write-Host "  Docker Engine da cai xong: $dockerVer" -ForegroundColor Green
} else {
    $dockerVer = docker --version
    Write-Host "  Docker da co san: $dockerVer" -ForegroundColor Green
}
...
if (!$composeOk) {
    ...
    $composeDir = Join-Path $env:ProgramFiles 'Docker\cli-plugins'
    New-Item -ItemType Directory -Force -Path $composeDir | Out-Null
    $composeDest = Join-Path $composeDir 'docker-compose.exe'
    Invoke-WebRequest -Uri $composeUrl -OutFile $composeDest
...
$projectDir = Join-Path $HOME 'codex-ai'
...
if (!(Test-Path $cliproxyExe)) {
    ...
    try {
        ...
        if ($asset) {
            ...
            $downloadFile = Join-Path $env:TEMP 'cliproxyapi.zip'
...
server:
  ...
  host: 127.0.0.1
'@
        $configContent | Out-File -FilePath $cliproxyConfig -Encoding ASCII
    }
...
Write-Host "    [OK] CLIProxyAPI: $cliproxyExe" -ForegroundColor Green
